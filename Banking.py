...
def make_deposit(bank_account, account, money):
    ...
    print(f"\nDeposited ${'{:,}'.format(money)} into {bank_account['Name']}'s {account.lower()} account.")


def make_withdrawal(bank_account, account, money):
    """Withdraw money from a specific type of account"""
    # Check that the balance will still be positive after withdrawal
    if bank_account[account] - money >= 0:
        bank_account[account] -= money
        print(f"\nWithdrew ${'{:,}'.format(money)} from {bank_account['Name']}'s {account.lower()} account.")
    # Not enough money to make the withdrawal
    else:
...
while running:
    ...
    # Make the correct function call based on user input
